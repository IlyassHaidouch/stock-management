<?php
	...
	if($categorie=="all"){
		$requete = "select * from entrees where designation like '%$nomM%' limit $size offset $offset";
		$requeteCount = "select count(*) countM from entrees where designation like '%$nomM%'";
	}
	else{
		$requete = "select * from entrees where designation like '%$nomM%' and categorie='$categorie' limit $size offset $offset";
		$requeteCount = "select count(*) countM from entrees where designation like '%$nomM%' and categorie='$categorie'";
	}
	
	$resultatM = $pdo->query($requete);
	$resultatCount = $pdo->query($requeteCount);
	$tabCount = $resultatCount->fetch();
	$nbrEntrees = $tabCount['countM'];

	$reste = $nbrEntrees%$size;
	if($reste===0){
		$nbrPage = $nbrEntrees/$size;
	}
	else{
		$nbrPage = floor($nbrEntrees/$size)+1; //la partie entière d'un nombre décimal
	}
?>
...
<body>  
	...
	<div class="container">
		<div class="panel panel-success margetop">
			<div class="panel-heading">Rechercher les entrées</div>

			<div class="panel-body">
				<form method="get" action="entrees.php" class="form-inline">
					...
					<?php if($_SESSION['user']['role']=='ADMIN'){?>
						<a href="nouvelleEntree.php"><span class="glyphicon glyphicon-plus"></span> Nouvelle marchandise</a>
					<?php } ?>	
				</form>
			</div>
	...
	<div class="container">
		<div class="panel panel-primary">
			<div class="panel-heading">Listes des entrées (<?php echo $nbrEntrees;?> Marchandises)</div>
			...
			<div class="panel-body">
				<table class="table table-striped table-bordered">
					<thead>
						<tr>
							<th>Numéro</th> <th>Date</th> <th>Référence</th> <th>Désignation</th> <th>Catégorie</th><th>Quantité</th> <th>Coût d'achat</th> <th>Total</th><?php if($_SESSION['user']['role']=='ADMIN'){?><th>Actions</th><?php } ?>
						</tr>
					</thead>
					...
					<tbody>
							<?php while($marchandise=$resultatM->fetch()){ ?>
								...
								<tr>
									...
									<td><?php echo $marchandise['cout_achat'].' DH';?> </td>
									<td><?php echo $marchandise['total'].' DH';?> </td>
									<?php if($_SESSION['user']['role']=='ADMIN'){?>
										<td>
											<a  href="editerEntrees.php?numero=<?php echo $marchandise['numero'];?>">
												<span class="glyphicon glyphicon-edit"></span>
											</a>
											&nbsp &nbsp

											<a onclick="return confirm('Etes vous sur vouloir supprimer la marchandise')" href="supprimerEntrees.php?numero=<?php echo $marchandise['numero'];?>">
						...
						<ul class="pagination pagination-md">
							<?php for($i=1; $i <= $nbrPage; $i++){ ?>
								   <li class="<?php if($page==$i){ echo 'active'; } ?>">
									   	<a href="entrees.php?page=<?php echo $i;?>&categorie=<?php echo $categorie;?>">
									   	<?php echo $i;?>
									    </a>
								   </li>  
							<?php } ?>	   
